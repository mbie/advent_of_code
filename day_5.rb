...
def count_steps(input)
  ...
  begin 
    ...
    index += value
    count += 1
  end while index < input.size 
  count
end

puts count_steps([0, 3, 0, 1, -3]) # => 5
puts count_steps(input.split("\n").map(&:to_i)) # => 387096
...
def count_steps(input)
  ...
  begin 
    value = input[index]
    modifier = value >= 3 ? -1 : 1
    input[index] += modifier
    index += value
    count += 1
  end while index < input.size 
  count
